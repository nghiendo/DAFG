...
def process(filename):
    ...
    fout1 = open(os.path.join(filepath, f'{base_filename}_write.json_distance.pkl'), 'wb')
    fout2 = open(os.path.join(filepath, f'{base_filename}_write.json_relation.pkl'), 'wb')
    fout3 = open(os.path.join(filepath, f'{base_filename}_write.json_opinion.pkl'), 'wb')

    for i in tqdm(range(0, len(lines), 3)):
        text = lines[i].strip()
        all_aspect = lines[i + 1].strip()
        start, end = aspect_indices(text)
        all_aspect = f"{all_aspect}#{start}#{end}"
        aspects = []
...
if __name__ == '__main__':
    process('./dataset/Laptops_corenlp/train.raw')
    process('./dataset/Laptops_corenlp/test.raw')
    process('./dataset/Restaurants_corenlp/train.raw')
    process('./dataset/Restaurants_corenlp/test.raw')
    process('./dataset/Tweets_corenlp/train.raw')
    process('./dataset/Tweets_corenlp/test.raw')

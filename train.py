...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            trainset = ABSAGCNData(opt.dataset_file['train'], opt.lexicon_file['train'], tokenizer, opt=opt)
            testset = ABSAGCNData(opt.dataset_file['test'], opt.lexicon_file['test'], tokenizer, opt=opt)
        else:
...
def main():
    ...
    input_colses = {
        'newbert': ['text_bert_indices', 'text_prompt_indices', 'aspect_bert_indices', 'adj_matrix', 'edge_adj', 'distance_adj', 'relation_adj',
    ...
    parser.add_argument('--model_name', default='newbert', type=str, help=', '.join(model_classes.keys()))
    parser.add_argument('--dataset', default='restaurant', type=str, help=', '.join(dataset_files.keys()))
    parser.add_argument('--lexicon', default='restaurant', type=str, help=', '.join(lexicon_files.keys()))
    ...
    parser.add_argument('--beta', default=0.6, type=float, help='the threshold that whether link aspect with words directly')
    ...
    parser.add_argument('--roberta_dim', type=int, default=50265)
    ...
    opt.amr_edge_stoi = './models/stoi.pt'
    opt.amr_edge_pt = './models/embedding.pt'
    ...
    opt.lexicon_file = lexicon_files[opt.dataset]
